...
export async function exportSubscriptionProofBinder(subscription: Subscription): Promise<void> {
  try {
    ...
    doc.text('CancelProof - Cancellation Proof Binder', pageWidth / 2, yPosition, { align: 'center' });
    ...
    if (subscription.proofDocuments.length > 0) {
      ...
      doc.setFontSize(9);
      doc.setTextColor(100);
      doc.text('Note: File attachments are stored in CancelProof and can be accessed via the application.', 14, yPosition);
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Page ${i} of ${pageCount} - Generated by CancelProof`,
        pageWidth / 2,
    ...
    // Download the PDF
    const fileName = `CancelProof-${subscription.name.replace(/[^a-z0-9]/gi, '_')}-${format(new Date(), 'yyyy-MM-dd')}.pdf`;
    doc.save(fileName);
  } catch (error) {
...
export async function exportAllSubscriptionsProofBinder(subscriptions: Subscription[]): Promise<void> {
  ...
  doc.setFontSize(24);
  doc.setFont('helvetica', 'bold');
  doc.text('CancelProof', pageWidth / 2, yPosition, { align: 'center' });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(150);
    doc.text(
      `Page ${i} of ${pageCount} - CancelProof Binder`,
      pageWidth / 2,
      doc.internal.pageSize.getHeight() - 10,
      { align: 'center' }
    );
  ...
  // Download
  const fileName = `CancelProof-Complete-Binder-${format(new Date(), 'yyyy-MM-dd')}.pdf`;
  doc.save(fileName);
}